...
- 외부 트랜잭션이 롤백되면서 전체 내용은 모두 롤백된다.
- 트랜잭션 매니저는 롤백 시점에 신규 트랜잭션 여부에 따라 다르게 동작한다. 외부 트랜잭션은 신규 트랜잭션이다. 따라서 DB 커넥션에 실제 롤백을 호출한다.

### 내부 롤백
- 내부 트랜잭션에서 롤백을 트랜잭션 매니저에 요청하더라도 신규 트랜잭션이 아니기 때문에 실제 롤백을 호출하지 않는다.
- 실제 커넥션에 커밋이나 롤백을 호출하면 물리 트랜잭션이 끝나버린다.
- 아직 트랜잭션이 끝난 것이 아니기 때문에 실제 롤백을 호출하면 안 된다.
- 물리 트랜잭션은 외부 트랜잭션을 종료할 때 까지 이어져야한다.
- 트랜잭션 매니저는 외부 트랜잭션 즉, 신규 트랜잭션을 커밋하기 전 트랜잭션 동기화 매니저에 롤백 전용(`rollbackOnly=true`) 표시가 있는지 확인한다.
- 롤백 전용 표시가 있으면 물리 트랜잭션을 커밋하는 게 아니라 롤백한다.
- 실제 데이터베이스에 롤백이 반영되고, 물리 트랜잭션도 끝난다.
- 스프링은 `UnexpectedRollbackException` 런타임 예외를 던진다. 그래서 커밋을 시도했지만 기대하지 않는 롤백이 발생했다는 것을 명확하게 알려준다.

### REQUIRES_NEW
- 물리 트랜잭션을 분리하려면 내부 트랜잭션을 시작할 때 `REQUIRES_NEW` 옵션을 사용하면 된다.
- 외부 트랜잭션과 내부 트랜잭션이 각각 별도의 물리 트랜잭션을 가진다.
- 별도의 물리 트랜잭션을 가진다는 뜻은 DB 커넥션을 따로 사용한다는 뜻이다.
- 이 경우 내부 트랜잭션이 롤백되면서 내부 트랜잭션 로직이 롤백돼도 외부 트랜잭션 로직에서 저장한 데이터에는 영향을 주지 않는다.
- 최종적으로 내부 트랜잭션 로직은 롤백되고, 외부 트랜잭션 로직은 커밋된다.
- `REQUIRES_NEW` 옵션을 사용하면 물리 트랜잭션이 명확하게 분리된다.
- 다만 그만큼 데이터베이스 커넥션을 동시에 여러 개 사용한다는 점을 주의해야한다.
- 데이터베이스 커넥션이 더 빨리 고갈될 수 있다.

## 트랜잭션 분리하기
- 서비스에서 여러 레포를 호출할 때, 특정 레포의 트랜잭션을 참여시키지 않고 분리하고 싶은 경우가 있을 수 있다.
- 이때 두 가지 방법이 있다.
1. `REQUIRES_NEW`를 옵션에 추가하기
   - 장점: 구조를 크게 변경하지 않고도 적용할 수 있다.
   - 단점: 동시에 커넥션을 더 많이 사용하게 된다.
2. 서비스 메서드에서 동시에 호출하지 않기
   - `REQUIRES_NEW`를 옵션에 추가하기와 장단점이 반대이다.
