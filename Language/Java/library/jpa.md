...
- OSIV 전략은 트랜잭션 시작처럼 최초 DB 커넥션 시작 시점부터 API 응답이 끝날 때까지 영속성 컨텐스트와 DB 커넥션을 유지한다.
- 그래서 뷰 템플릿이나 API 컨트롤러에서 lazy 로딩이 가능했었다.
- lazy 로딩은 영속성 컨텍스트가 살아있어야 가능하고 영속성 컨텍스트는 기본적으로 데이터 베이스 커넥션을 유지한다.
- 그런데 오랜 시간동안 DB 커넥션 리소스를 사용하기 때문에 실시간 트래픽이 중요한 애플리케이션에서는 커넥션이 모자랄 수 있다.
  - 예를 들면 외부 API를 호출하는데 몇 초가 걸린다면 해당 시간만큼 커넥션을 계속 들고 있는다.
- 스프링에서는 기본적으로 `open-in-view: true`로 해당 속성을 켜놓는다.
- OSIV를 끄면 트랜잭션을 종료할 때 영속성 컨텍스트를 닫고 데이터베이스 커넥션도 반환한다. 그래서 커넥션 리소스를 낭비하지 않는다.
- 하지만 이렇게 되면 lazy 로딩을 트랜잭션 안에서 처리해야한다. 그렇지 않으면 `could not initialize proxy` 에러가 발생한다.
- 해결 방법은 별도의 서비스에서 즉, 별도의 트랜잭션 안에서 lazy 로딩을 처리하는 것이다. (쿼리용 서비스)
- 서비스에서 핵심 비즈니스 로직과 화면이나 API에 맞춘 로직을 분리하는 것이 유지보수 측면에서 낫다.
  - 예를 들어 Order 서비스가 있다면 Order 쿼리 서비스를 따로 만들어 여기에 읽기 전용 트랜잭션을 넣는 것이다.
- 실무에서는 실시간 API는 OSIV를 끄고 어드민처럼 커넥션을 많이 사용하지 않는 곳에서는 OSIV를 키는 것을 권장한다고 한다.
